 ...
 * upstream the number of buffers in the batched buffer.
 *
 ...
 *
 * @return A boolean indicating whether the query is a batch size query.
 */
gboolean gst_nvquery_is_batch_size (GstQuery * query);

/**
 * Sets the batch size, which is used by the elements responding to the batch size
 * query.
 *
 * This function fails if the query is not a batch size query.
 *
 * params[in] query A batch size `GstQuery`.
 ...
 *
 * params[in] query A batch size `GstQuery`.
 * params[out] batch_size A pointer to an unsigned integer in which the function
 *                        will places the batch size.
 *
...
/**
 * Creates a number of streams query, which can be used by elements to query
 ...
 * params[in] query The `GstQuery` to be checked.
 *
 * @return A boolean indicating if the query is a number of streams query.
 */
gboolean gst_nvquery_is_numStreams_size (GstQuery * query);

/**
 * Sets the number of input sources. This is used by the elements responding to
 * a number of streams query. The function fails if the query is not of the
 * correct type.
 *
 * params[in] query A number of streams `GstQuery`.
 ...
 * params[in] query A number of streams `GstQuery`.
 * params[out] batch_size A pointer to an unsigned integer in which the function
 *                        is to place the number of streams.
 *
